 ...
 * パフォーマンス統計エンドポイント
 */
export const performanceStatsHandler = async (c: Context<{ Bindings: Bindings }>) => {
  try {
    ...
    return c.json({
      ...
      data: {
        ...
        analysisLevels: {
          ...
          level1Rate: Math.round((stats.level1Count / stats.totalRequests) * 100),
          level2Rate: Math.round((stats.level2Count / stats.totalRequests) * 100),
          level3Rate: Math.round((stats.level3Count / stats.totalRequests) * 100),
 ...
 * ヘルスチェックエンドポイント（簡易版）
 */
export const healthCheckHandler = async (c: Context<{ Bindings: Bindings }>) => {
  try {
    const performanceMonitor = getPerformanceMonitor();
    const health = performanceMonitor.getHealthStatus();
    const stats = performanceMonitor.getStats();

    const isHealthy = health.status === 'healthy' && stats.p95ProcessingTime <= 10;

    return c.json({
      status: isHealthy ? 'healthy' : health.status,
      message: health.message,
      cpuWithinLimits: stats.p95ProcessingTime <= 10,
      averageResponseTime: Math.round(stats.averageProcessingTime * 100) / 100,
      successRate: Math.round(stats.successRate * 100) / 100,
      timestamp: new Date().toISOString()
 ...
 */
export const metricsExportHandler = async (c: Context<{ Bindings: Bindings }>) => {
  try {
    const performanceMonitor = getPerformanceMonitor();
    const csvData = performanceMonitor.exportMetricsAsCSV();

    c.header('Content-Type', 'text/csv');
    c.header('Content-Disposition', 'attachment; filename="performance-metrics.csv"');
 ...
 */
export const clearMetricsHandler = async (c: Context<{ Bindings: Bindings }>) => {
  try {
    const performanceMonitor = getPerformanceMonitor();
