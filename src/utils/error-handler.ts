...
export class ErrorHandler {
  ...
  public classifyError(error: Error, context?: string): ErrorDetails {
    // OpenAI APIエラー
    if (error.name === ERROR_NAMES.OPENAI_ERROR || error.message.includes('OpenAI')) {
    ...
    // ネットワークエラー
    if (error.name === ERROR_NAMES.TIMEOUT_ERROR || 
  ...
  private classifyOpenAIError(error: Error): ErrorDetails {
    ...
    if (statusCode === 429) {
      const retryAfter = (error as any).rateLimitReset;
      return {
  ...
  private getCircuitBreaker(key: string): CircuitBreakerState {
    if (!this.circuitBreakers.has(key)) {
      this.circuitBreakers.set(key, new CircuitBreakerState(this.defaultCircuitConfig));
    }
  ...
  async executeWithRetry<T>(
    ...
    for (let attempt = 0; attempt <= retryConfig.maxRetries; attempt++) {
      try {
        ...
        if (errorDetails.category === ErrorCategory.RATE_LIMIT && errorDetails.retryAfter) {
        ...
        console.warn(`Retrying operation "${context}" (attempt ${attempt + 1}/${retryConfig.maxRetries + 1}):`, {
    ...
    // 到達しないコードパス（型安全性のため）
    throw new EnhancedError(lastError!.message, this.classifyError(lastError!, context));
  }
  ...
  private logError(
    ...
    const logLevel = errorDetails.category === ErrorCategory.TEMPORARY ? 'warn' : 'error';
    const logData = {
      context,
      attempt,
 ...
 */
export function getUserMessageFromError(error: Error, context?: string): string {
