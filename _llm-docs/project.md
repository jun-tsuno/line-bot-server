...
### リアルタイムログ監視
- **本番ログ監視**: `wrangler tail` でデプロイ済みWorkerのリアルタイムログを確認
- **フォーマット指定**: `wrangler tail --format=pretty` で見やすい形式で表示
- **使用場面**: 
  - AI分析エラーの詳細確認
  - LINE Webhook処理の動作確認
  - バッチ処理の実行状況確認
...
### ログ監視の手順
1. ターミナルで `wrangler tail` を実行
2. LINE Botに日記を送信してAI分析を実行
3. エラーまたは処理ログがリアルタイムで表示される
4. `Ctrl+C` で監視を終了
